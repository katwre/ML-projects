...
from app.custom_strategy import StoreLossSaveOnFinalFedAvg, StoreBothLossesSaveOnFinalFedAvg
...
def server_fn(context: Context):
    ...
    #In federated learning using FedAvg:
    #Each client trains locally on its data → computes training loss.
    #Then, the server aggregates the weights.
    #After aggregation, the model is evaluated (often on a validation set, sometimes on a held-out test set).
    #By default, the framework only logs one scalar metric → usually the validation loss from the server’s evaluation.
    #This means that if you only rely on the default setup, you won’t see the local training losses on the clients during aggregation rounds.

    ## StoreBothLossesSaveOnFinalFedAvg
